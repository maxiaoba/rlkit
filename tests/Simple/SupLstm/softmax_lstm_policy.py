...
class SoftmaxLSTMPolicy(Policy, nn.Module):
    ...
    def step(self, obs_action, return_info=False):
        ...
        obs = obs[:,None,:] # batch x 1 x |O|
        prev_actions = prev_actions[:,None,:] # batch x 1 x 1
        prev_actions = self.to_onehot(prev_actions)
        obs = torch.cat((obs,prev_actions),dim=-1)
    ...
    def get_action(self, obs, deterministic=False):
        ...
        with torch.no_grad():
            obs_action = (torch_ify(obs)[None], self.a_p[None])
            pis, info = self.step(obs_action, return_info=True)
        pis = np_ify(pis[0])
        if deterministic:
...
if __name__ == '__main__':
    ...
    print(policy.h_p)
    print(policy.c_p)
    obs_batch = torch.zeros((8,6,obs_dim))
    pre_action_batch = torch.zeros((8,5,action_dim))
    pis = policy((obs_batch,pre_action_batch))
    print(pis.shape)
