...
def experiment(variant):
    ...
    decoder = nn.Sequential(
            ConcatLayer([
                nn.Sequential(nn.Linear(obs_dim,16),nn.ReLU()),
                nn.Sequential(sup_learner,FlattenLayer()),
                ],need_gradients=[True, (not variant['no_gradient'])]),
            nn.Linear(16+action_dim, action_dim),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', type=str, default='TRPOSup3')
    parser.add_argument('--ng', action='store_true', default=False)
