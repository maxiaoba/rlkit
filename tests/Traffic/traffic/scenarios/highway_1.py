...
class EnvDriver(XYSeperateDriver):
    ...
    def observe(self, cars, road):
        ...
        if not self.aggressive:
            ...
            if y > 4.0:
                self.x_driver.p_des = x + np.minimum(np.random.rand(), min_front_distance1 - self.min_front_x)
            else:
                self.x_driver.p_des = x + np.minimum(np.random.rand(), min_front_distance0 - self.min_front_x)
        else:
...
class EgoDriver(XYSeperateDriver):
    def __init__(self, 
        ...
        x_driver = IDMDriver(sigma=x_sigma, v_des=10.0, s_des=1.0, s_min=0.5, axis=0, min_overlap=0., **kwargs)
...
class HighWay(TrafficEnv):
    ...
    def update(self, action):
        ...
        for _ in range(self.num_updates):
            ...
            if min_lower_x > (self.left_bound + np.random.uniform(self.gap_min,self.gap_max) + self.car_length):
                ...
                car, driver = self.add_car(x, y, 0., 0., x_des, y_des, aggressive, 0.)
    ...
    def _reset(self):
        ...
        car = Car(idx=0, length=self.car_length, width=self.car_width, color=random.choice(BLUE_COLORS),
                          ...
                          max_rotation=self.car_max_rotation,
                          expose_level=self.car_expose_level)
        driver = EgoDriver(v_des=0., y_des=y_0,
                            x_sigma=self.driver_sigma, y_sigma=0.,
