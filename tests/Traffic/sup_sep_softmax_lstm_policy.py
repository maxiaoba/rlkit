...
class SupSepSoftmaxLSTMPolicy(Policy, nn.Module):
    ...
    def to_policy_inputs(self, obs_action, labels=None, return_info=False):
        ...
        obs = torch.cat((obs,onehot_labels),dim=-1).reshape(*obs_flat.shape[:-1],-1)
    ...
    def forward(self, obs_action, labels, return_info=False):
        policy_inputs, sup_info = self.to_policy_inputs(obs_action, labels=labels, return_info=True)
